...
for glyph in ['a', 'b']:
    ...
    def drawRect(pen, x, y, size=50):
        pen.moveTo((x-.9*size, y-.9*size))
        pen.lineTo((x+.9*size, y-.9*size))
        pen.lineTo((x+.9*size, y+.9*size))
        pen.lineTo((x-.9*size, y+.9*size))
        pen.closePath()
    ...
    yValues = range(int(yMin) - 30 - int(yMin) / resolution, int(yMax), resolution)
    yValues.reverse()
    ...
    for y in yValues:
        for x in range(int(xMin), int(xMax), resolution):
            ...
            if source.pointInside((x, y)):
                drawRect(myPen, x, y+20, resolution-40)
